...
def main():
    df = pd.read_csv("./archive/cost-of-living.csv")
    documents = []
    for _, row in df.iterrows():
        lines = [f'Cost of living in {row["city"]}, {row["country"]}:']
        
        for col, hum_name in data_dict.items():
            val = row.get(col)
            if pd.notna(val):
                lines.append(f'- {hum_name}: {val}')
    ...
    collection = client.create_collection(name="cost_of_living", embedding_function=embed_func)
    collection.add(
        ids=[f'city_{i}' for i in range(len(documents))],
        documents = documents
    )
    print("ffff")
